...
import {
  ...
  newEvent, newSnapshot, newPrepEntry,
  calcMetrics,
  EVENT_TYPES,
  normalizeName, resolveBatchNames, mergePlayerObjects,
  parseHMS, formatHMS, secsToHuman,
...
function newPlayer(o={}) {
  return {id:uid(),fid:"",username:"",alias:"",allianceTag:"",country:"",timezone:"",languages:[],furnaceLevel:null,infantryCampLevel:null,lancerCampLevel:null,marksmanCampLevel:null,troops:{infantry:null,lancer:null,marksman:null},joinerHeroes:[],heroes:[],hasNoneChecked:false,roles:[],availability:{present:"available",timing:"unknown",lateBy:null,earlyBy:null,discord:"unknown"},teamAssignment:null,notes:"",profileLastUpdated:null,createdAt:Date.now(),...o};
}

// ── Tutorial data ──────────────────────────────────────────────
const TUTORIAL_STEPS = {
  beginner: [
    { id:"welcome",    title:"Welcome to Sunfire Command",          body:"Your command center for Whiteout Survival SvS. Manage your roster, coordinate battle plans, track attendance, and analyze performance — all from your phone.", why:"Alliance coordination without a central tool leads to missed rallies, wrong timing, and confusion.", how:"Use this app before and during every SvS. Officers update it in real time.", when:"Set up your roster before the season starts." },
    { id:"roster",     title:"👥 Roster — Your Player Database",    body:"Add every player in your alliance. Each profile stores username, troop tiers, joiner heroes, role, and availability.", why:"Knowing your players' stats lets you assign the right people to the right jobs.", how:"Tap ＋ to add a player, or use ⚡ Batch Add to add many at once.", when:"Build your roster before every SvS." },
    { id:"batch",      title:"⚡ Batch Add — Fast Roster Entry",    body:"Paste a list of names and the app walks you through tiers, heroes, and availability for all of them. Detects duplicates automatically.", why:"Entering players one by one is slow. Batch Add gets your roster ready in minutes.", how:"Tap ⚡ Batch → type or paste names → review → set tiers and heroes.", when:"Before each SvS to register who's participating." },
    { id:"battle",     title:"⚔️ Battle — Strategy Planning",       body:"Build battle plans, add rallies and reinforcements, assign teams, and run Live Mode during the battle with large countdowns.", why:"Uncoordinated rallies waste power. Plans ensure everyone knows their role.", how:"Tap Battle → New Battle Plan → add rallies and reinforcements.", when:"Build the plan 30 minutes before SvS. Switch to Live Mode at start." },
    { id:"events",     title:"📋 Events — Track Every Battle",      body:"Create an event for each SvS, Foundry, or Canyon Clash. Track who attended, who was on Discord, who performed well, and who no-showed.", why:"Tracking events builds historical data for rewarding reliable players.", how:"Events → New Event → fill in details → track players during the event.", when:"Create before it starts. Mark attendance during the battle." },
    { id:"scores",     title:"📈 Scores — Prep Score Tracking",     body:"Track each player's SvS prep score, target, and progress. Batch update by pasting a list. History is saved on each update.", why:"Prep scores determine event eligibility. Tracking them helps enforce requirements.", how:"Scores → Add or Batch → paste Name, Alliance, Score, Target per line.", when:"After the prep tracking deadline each season." },
    { id:"intel",      title:"📊 Intel — Alliance Performance",     body:"Reliability scores, attendance leaderboard, at-risk players, joiner hero coverage, and the Rally Joiner Registry all live here.", why:"Data helps leaders make fair decisions about roles, rewards, and roster management.", how:"Review Intel after each SvS. Check leaderboard before assigning rally leads.", when:"After each event and before each season." },
    { id:"export",     title:"📦 Export Your Data",                 body:"Tap 📦 in the top right to download all your data as a JSON file. Import it back later or share with another officer.", why:"Backups protect your work. Sharing keeps officers in sync.", how:"Tap 📦 → Download JSON. Keep the file safe.", when:"After building your roster. After each major event." },
  ],
  advanced: [
    { id:"counter",    title:"⚔️ Counter Rally Planning",           body:"After the enemy rally launches, mark their impact time and calculate when your counter must launch to land seconds after.", why:"A counter too early gets blocked. Too late and the enemy reinforces.", how:"Battle → plan → Counter Rally strategy → add enemy rally → add counter → check warnings.", when:"The moment an enemy rally is spotted." },
    { id:"warnings",   title:"Rally Sync Warnings",                  body:"The app automatically warns when rallies are >10s apart, when the strongest rally arrives first, or when a counter is mistimed.", why:"Sync failures let the enemy reinforce between your rallies.", how:"Warnings appear in red at the top of your plan automatically.", when:"Check before marking a plan Live." },
    { id:"reinf",      title:"🏰 Reinforcement Calculator",          body:"Enter target arrival time and march duration — the app calculates exact send time. Example: target 12:00:00, march 40s → send at 11:59:20.", why:"Wrong reinforcement timing blocks your solo attacker.", how:"Battle → plan → Reinforcements tab → fill in each row.", when:"During castle switch planning." },
    { id:"live",       title:"🔴 Live Battle Mode",                  body:"Large countdown to target impact. One-tap buttons to confirm each rally launched, each reinforcement sent, each team confirmed.", why:"During battle you need speed. Live Mode removes all clutter.", how:"Battle → open plan → tap Live Mode.", when:"The moment SvS starts." },
    { id:"registry",   title:"🦸 Rally Joiner Registry",            body:"Track who owns each joiner hero at Skill 5. View coverage gaps, check for stacking, and get meta formation recommendations by generation.", why:"Joiner hero data is essential for correct team assignments.", how:"Intel → Rally Joiner Registry → tap a hero card to see owners or add players.", when:"Before each SvS. After any hero events." },
    { id:"suggest",    title:"⚡ Auto-Suggest Assignments",          body:"Filter by required heroes, Discord, furnace level, reliability — the app ranks your players by suitability and lets you assign them directly.", why:"Picking teams from memory misses better candidates.", how:"Battle → open plan → Auto-Suggest → set filters → assign from results.", when:"Building teams 1–2 hours before SvS." },
  ],
  discovery: [
    { id:"registry2",  title:"🆕 Rally Joiner Registry",            body:"Replaces the old Hero Ownership entry. Track joiner heroes by generation, check coverage gaps, stacking warnings, and meta tables.", why:"The old system didn't connect hero data to team planning. This one does.", how:"Intel → Rally Joiner Registry.", when:"Any time hero data changes." },
    { id:"tabs",       title:"🆕 Redesigned Tab Structure",         body:"Teams view moved into Roster as a toggle. Plans moved to Battle tab at position 2. Stats renamed Intel.", why:"Fewer taps to reach what matters most during a live battle.", how:"Use the toggle in Roster to switch between List and By Role views.", when:"Any time." },
  ],
};
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:20,padding:24,margin:"0 16px",width:"100%",maxWidth:440,position:"relative",boxShadow:"0 20px 60px #000a"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.1em"}}>
            {mode==="beginner"?"Beginner":mode==="advanced"?"Advanced":mode==="discovery"?"What's New":""} · Step {progress} of {total}
          </div>
        ...
        {step.why&&<div style={{background:C.section,borderRadius:10,padding:12,marginBottom:8}}><div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>Why it matters</div><div style={{fontSize:13,color:C.icy,lineHeight:1.5}}>{step.why}</div></div>}
        {step.how&&<div style={{background:C.section,borderRadius:10,padding:12,marginBottom:8}}><div style={{fontSize:11,fontWeight:700,color:C.green,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>How to use it</div><div style={{fontSize:13,color:C.icy,lineHeight:1.5}}>{step.how}</div></div>}
        {step.when&&<div style={{background:C.section,borderRadius:10,padding:12,marginBottom:16}}><div style={{fontSize:11,fontWeight:700,color:C.icy,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>When to use</div><div style={{fontSize:13,color:C.icy,lineHeight:1.5}}>{step.when}</div></div>}
        <div style={{display:"flex",gap:10,marginTop:16}}>
          {idx>0&&<button onClick={()=>setIdx(i=>i-1)} style={{flex:1,height:48,borderRadius:12,background:C.section,border:`1px solid ${C.border}`,color:C.icy,fontWeight:600,fontSize:15,cursor:"pointer"}}>← Back</button>}
          {idx<steps.length-1
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",padding:"24px 20px 60px"}}>
        ...
        {[
          ...
          <button key={mode} onClick={()=>onSelect(mode)} style={{display:"flex",alignItems:"center",gap:14,width:"100%",background:C.section,borderRadius:14,padding:"16px 18px",marginBottom:10,border:`1px solid ${color}33`,cursor:"pointer",textAlign:"left"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:700,color:C.white}}>{title}</div><div style={{fontSize:13,color:C.muted,marginTop:2}}>{sub}</div></div>
...
function LandingPage({ onGetStarted, onContinue, onImport, onTutorial, hasData }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",fontFamily:"system-ui,-apple-system,sans-serif",textAlign:"center"}}>
      ...
      <div style={{fontSize:28,fontWeight:800,color:C.white,marginBottom:8,lineHeight:1.2,opacity:animIn?1:0,transition:"all 600ms ease 100ms"}}>Sunfire Command</div>
      <div style={{fontSize:16,color:C.muted,marginBottom:32,lineHeight:1.6,maxWidth:320,opacity:animIn?1:0,transition:"all 600ms ease 150ms"}}>Build your alliance roster, assign rally teams, and plan your Sunfire Castle battle.</div>
      <div style={{background:C.section,borderRadius:16,padding:"20px 24px",marginBottom:36,width:"100%",maxWidth:360,textAlign:"left",opacity:animIn?1:0,transition:"all 600ms ease 200ms"}}>
        {[["👥","Add your members and stats"],["⚔️","Assign rally and reinforcement teams"],["📋","Plan switching, rallies, and timelines"],["🎙️","Track Discord voice participation"],["📊","Analyze SvS performance history"]].map(([icon,text])=>(
      ...
      <div style={{width:"100%",maxWidth:360,opacity:animIn?1:0,transition:"all 600ms ease 250ms"}}>
        ...
        {hasData&&<button onClick={onContinue} style={{width:"100%",height:52,borderRadius:14,background:C.section,border:`1px solid ${C.border}`,color:C.white,fontWeight:700,fontSize:16,cursor:"pointer",marginBottom:10}}>Continue Existing Plan</button>}
        <button onClick={onImport} style={{width:"100%",height:52,borderRadius:14,background:C.section,border:`1px solid ${C.border}`,color:C.icy,fontWeight:600,fontSize:16,cursor:"pointer",marginBottom:20}}>⬆️ Import Data</button>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onTutorial} style={{flex:1,height:44,borderRadius:12,background:"none",border:`1px solid ${C.border}`,color:C.muted,fontWeight:600,fontSize:14,cursor:"pointer"}}>📖 Tutorial</button>
          <button onClick={onGetStarted} style={{flex:1,height:44,borderRadius:12,background:"none",border:`1px solid ${C.border}`,color:C.muted,fontWeight:600,fontSize:14,cursor:"pointer"}}>⚡ Quick Demo</button>
        </div>
      </div>
...
function PlayerPicker({ players, value, onChange, placeholder="Search players…", clearable=true, style={} }) {
  ...
  const results = query.trim()
    ? players.filter(p=>{
        ...
        return [p.username,p.alias,p.fid,p.allianceTag].map(f=>normalizeName(f||"")).some(f=>f.includes(q));
      }).slice(0,8)
    : [];
  function select(player){onChange(player?.id||null,player||null);setQuery("");setOpen(false);}
  function clear(){onChange(null,null);setQuery("");setOpen(false);}
  useEffect(()=>{
  ...
  return (
    <div ref={containerRef} style={{position:"relative",...style}}>
      {selectedPlayer?(
        ...
        <div>
          <input ref={inputRef} value={query} onChange={e=>{setQuery(e.target.value);setOpen(true);}} onFocus={()=>setOpen(true)} placeholder={placeholder} style={{width:"100%",background:C.section,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",fontSize:16,color:C.white,boxSizing:"border-box",fontFamily:"inherit"}}/>
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"#000c",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"70vh",overflowY:"auto",padding:"16px 20px 60px"}}>
        ...
        {owners.length===0
          ?<div style={{textAlign:"center",padding:"30px 0",color:C.muted}}>No players own this hero at Skill 5</div>
          :owners.map(p=>(
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"#000c",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"92vh",overflowY:"auto",padding:"16px 20px 80px"}}>
        ...
        {/* Joiner Heroes — reads from joinerHeroes */}
        {(player.joinerHeroes||[]).filter(jh=>jh.skillLevel>=5).length>0 ? (
          <div style={{background:C.section,borderRadius:12,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {(player.joinerHeroes||[]).filter(jh=>jh.skillLevel>=5).map(jh=>(
                <span key={jh.hero} style={{padding:"6px 14px",borderRadius:20,background:C.gold+"18",border:`1px solid ${C.gold}44`,color:C.gold,fontWeight:600,fontSize:14}}>
                  ✓ {jh.hero}{jh.verified?" ✓":""}
                </span>
              ))}
            </div>
          ...
          <div style={{background:C.section,borderRadius:12,padding:14,marginBottom:12}}>
            <div style={{fontSize:13,color:C.muted}}>No joiner heroes recorded — update in 🦸 Joiner Registry</div>
          </div>
        )}
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"92vh",overflowY:"auto",padding:"16px 20px 100px"}}>
        ...
        {activeTab==="avail"&&<div>
          <Field label="SvS Availability">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[["✅ Available","available",C.green],["❌ Unavailable","unavailable",C.red]].map(([l,v,c])=><button key={v} onClick={()=>updA({present:v})} style={{height:52,borderRadius:12,border:`1px solid ${p.availability.present===v?c:C.border}`,background:p.availability.present===v?c+"18":C.section,color:p.availability.present===v?c:C.muted,fontWeight:600,fontSize:15,cursor:"pointer"}}>{l}</button>)}
            </div>
          </Field>
          <Field label="Timing">
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {[["⏰ On Time","on-time"],["🕐 Late","late"],["🚪 Leaving Early","early"],["❓ Unknown","unknown"]].map(([l,v])=><button key={v} onClick={()=>updA({timing:v})} style={{padding:"8px 14px",borderRadius:20,minHeight:40,border:`1px solid ${p.availability.timing===v?C.gold:C.border}`,background:p.availability.timing===v?C.gold+"18":C.section,color:p.availability.timing===v?C.gold:C.muted,fontWeight:600,fontSize:14,cursor:"pointer"}}>{l}</button>)}
            </div>
          </Field>
          <Field label="Discord During SvS">
            <div style={{display:"flex",gap:8}}>
              {[["🎙️ On Discord","yes"],["🔇 Not on Discord","no"],["❓ Unknown","unknown"]].map(([l,v])=><button key={v} onClick={()=>updA({discord:v})} style={{flex:1,height:44,borderRadius:12,border:`1px solid ${p.availability.discord===v?C.icy:C.border}`,background:p.availability.discord===v?C.icy+"18":C.section,color:p.availability.discord===v?C.icy:C.muted,fontWeight:600,fontSize:13,cursor:"pointer"}}>{l}</button>)}
            </div>
          </Field>
        </div>}
...
function BatchAddSheet({ open, onClose, members, onAddNew, onUpdateExisting }) {
  ...
  function updateSuggestions(text) {
    ...
    setSuggestions(members.filter(p=>[p.username,p.alias,p.fid,p.allianceTag].map(f=>normalizeName(f||"")).some(f=>f.includes(q))).slice(0,6));
  }
  function addLine(text,linkedId=null){if(!text.trim())return;setRawLines(prev=>[...prev,{text:text.trim(),linkedId}]);setInputText("");setSuggestions([]);}
  function removeLine(idx){setRawLines(prev=>prev.filter((_,i)=>i!==idx));}
  function handleInputKey(e){if(e.key==="Enter"||e.key===","){e.preventDefault();addLine(inputText);}}
  ...
  function resetAll(){setPhase(0);setRawLines([]);setInputText("");setSuggestions([]);setTagAll("");setTzAll("");setShowOpt(false);setResolved(null);setFuzzyDec({});setVoiceSet(new Set());setLateSet(new Set());setLateBy("unknown");setEarlySet(new Set());setEarlyBy("unknown");setUnavailSet(new Set());setGrpTierSel(new Set());setGrpTroops({infantry:null,lancer:null,marksman:null});setMemTroops({});setTierIdx(0);}
  ...
  function resolve(){
    ...
    linked.forEach(l=>{const player=members.find(p=>p.id===l.linkedId);if(player)res.exact.push({name:l.text,existingPlayer:player});else res.fresh.push({name:l.text});});
    setResolved(res);
      ...
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"92vh",overflowY:"auto",padding:"16px 20px 80px"}}>
        ...
        {phase===0&&(
          <div>
            ...
            <div style={{fontSize:13,color:C.icy,marginBottom:16,lineHeight:1.6}}>Type names one at a time. Select from suggestions to link to existing players.</div>
            {rawLines.length>0&&(
            ...
            <div style={{position:"relative",marginBottom:12}}>
              <div style={{display:"flex",gap:8}}>
                <input ref={inputRef} value={inputText} onChange={e=>{setInputText(e.target.value);updateSuggestions(e.target.value);}} onKeyDown={handleInputKey} placeholder="Type name, press Enter to add…" style={{flex:1,background:C.section,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",fontSize:16,color:C.white,fontFamily:"inherit"}}/>
            ...
            <details style={{marginBottom:16}}>
              <summary style={{fontSize:13,color:C.gold,cursor:"pointer",padding:"4px 0"}}>Or paste a list ▸</summary>
              <textarea style={{width:"100%",minHeight:80,background:C.section,border:`1px solid ${C.border}`,borderRadius:10,padding:12,fontSize:15,color:C.white,resize:"none",boxSizing:"border-box",fontFamily:"inherit",lineHeight:1.8,marginTop:8}} placeholder={"Marcus, Caroline\nZhangWei"} onChange={e=>{const names=e.target.value.split(/[\n,]/).map(n=>n.trim()).filter(Boolean);names.forEach(n=>{if(!rawLines.some(l=>normalizeName(l.text)===normalizeName(n)))addLine(n);});e.target.value="";}}/>
            </details>
            ...
            <button disabled={rawLines.length===0} onClick={resolve} style={{width:"100%",height:54,borderRadius:12,background:rawLines.length>0?C.gold:C.border,color:C.bg,fontWeight:700,fontSize:17,border:"none",cursor:rawLines.length>0?"pointer":"default"}}>Continue with {rawLines.length} member{rawLines.length!==1?"s":""} →</button>
          </div>
        )}
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"#000c",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"88vh",overflowY:"auto",padding:"16px 20px 80px"}}>
        ...
        <Field label="Participating Players" hint="Select who's in this event">
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>{players.map(p=>{const sel=(ev.participantIds||[]).includes(p.id);return <button key={p.id} onClick={()=>{const cur=ev.participantIds||[];upd("participantIds",sel?cur.filter(id=>id!==p.id):[...cur,p.id]);}} style={{padding:"6px 12px",borderRadius:16,minHeight:36,border:`1px solid ${sel?C.gold:C.border}`,background:sel?C.gold+"22":C.section,color:sel?C.gold:C.muted,fontWeight:600,fontSize:13,cursor:"pointer"}}>{p.username||p.alias||"?"}</button>;})}</div>
    ...
    <div style={{padding:"16px 20px 0"}}>
      {activeEvent?(
        ...
        <>
          ...
          {sorted.length===0
            ?<div style={{textAlign:"center",padding:"60px 20px"}}><div style={{fontSize:40,marginBottom:12}}>📋</div><div style={{fontSize:16,fontWeight:700,color:C.white,marginBottom:8}}>No events yet</div></div>
            :sorted.map(ev=>{
...
function StatsTab({ players, events, onHeroClick, onHeroOwnership }) {
  ...
  return (
    <div style={{padding:"16px 20px"}}>
      <button onClick={onHeroOwnership} style={{width:"100%",height:52,borderRadius:12,background:C.section,border:`1px solid ${C.gold}44`,color:C.gold,fontWeight:700,fontSize:15,cursor:"pointer",marginBottom:16}}>🦸 Rally Joiner Registry</button>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        {[["👥","Total Players",players.length],["📋","Total Events",events.length],["👑","Rally Leads",players.filter(p=>p.roles?.includes("Rally Lead")).length],["✅","Available",players.filter(p=>p.availability?.present==="available").length],["🦸","With Joiners",players.filter(p=>(p.joinerHeroes||[]).some(jh=>jh.skillLevel>=5)).length],["🌏","Countries",new Set(players.map(p=>p.country).filter(Boolean)).size]].map(([i,l,v])=>(
          <div key={l} style={{background:C.card,borderRadius:12,padding:16}}><div style={{fontSize:22}}>{i}</div><div style={{fontSize:28,fontWeight:700,color:C.gold}}>{v}</div><div style={{fontSize:13,color:C.icy}}>{l}</div></div>
        ))}
      </div>
      {withM.length>0&&(
        <div style={{background:C.card,borderRadius:12,padding:16,marginBottom:16}}>
          ...
          {withM.slice(0,8).map(({player,metrics},i)=>(
            <div key={player.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:`1px solid ${C.border}22`}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.white}}>{player.username||player.alias||"?"}</div><div style={{fontSize:11,color:C.muted}}>{metrics.attended}/{metrics.totalEvents} · {metrics.attendancePct}%</div></div>
      ...
      {(()=>{
        ...
        return (
          <div style={{background:C.card,borderRadius:12,padding:16,marginBottom:16}}>
            <div style={{fontSize:15,fontWeight:700,color:C.white,marginBottom:4}}>🏅 Top Heroes (Skill 5)</div>
            <div style={{fontSize:12,color:C.muted,marginBottom:10}}>Tap to see who owns it</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {top.map(([hero,count])=>(
                <button key={hero} onClick={()=>onHeroClick(hero)} style={{padding:"8px 12px",borderRadius:20,background:C.gold+"18",border:`1px solid ${C.gold}33`,cursor:"pointer"}}>
            ...
            {sorted.map(([c,n])=>(
              <div key={c} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <div style={{width:80,height:6,borderRadius:3,background:C.border,overflow:"hidden"}}><div style={{width:`${(n/players.length)*100}%`,height:"100%",background:C.gold,borderRadius:3}}/></div>
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"#000c",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",padding:"16px 20px 60px",maxHeight:"80vh",overflowY:"auto"}}>
        ...
        <div style={{background:C.section,borderRadius:12,padding:16,marginBottom:16}}>
          ...
          <div style={{fontSize:13,color:C.muted,marginBottom:12}}>Downloads all data as JSON.</div>
...
export default function App() {
  ...
  const [rosterView, setRosterView]   = useState("list");   // ← fixed: moved out of JSX
  ...
  function handleGetStarted(){localStorage.setItem("svs_onboarded","1");setShowLanding(false);if(!data.players.length)setTimeout(()=>setTutorialMode("beginner"),400);}
  function handleLandingImport(){importFileRef.current?.click();}
  ...
  const TABS=[
    {icon:"👥",label:"Roster"},   // 0
    {icon:"⚔️",label:"Battle"},   // 1  — placeholder until SvsPlanTab wired
    {icon:"📋",label:"Events"},   // 2
    {icon:"📈",label:"Scores"},   // 3
    {icon:"📊",label:"Intel"},    // 4
  ];
  ...
  if(showLanding){
    return(
      <>
        <LandingPage hasData={players.length>0} onGetStarted={handleGetStarted} onContinue={handleGetStarted} onImport={handleLandingImport} onTutorial={()=>{setShowLanding(false);localStorage.setItem("svs_onboarded","1");setTutorialPickerOpen(true);}}/>
        <input type="file" accept=".json" ref={importFileRef} style={{display:"none"}} onChange={async e=>{const file=e.target.files?.[0];if(!file)return;try{const imp=await importData(file);setData(prev=>({...prev,...imp,lastUpdated:new Date().toISOString()}));handleGetStarted();showToast("Data imported ✓");}catch{showToast("Import failed","error");}e.target.value="";}}/>
    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.white,fontFamily:"system-ui,-apple-system,sans-serif",paddingBottom:80,maxWidth:480,margin:"0 auto"}}>
      ...
      <div style={{padding:"20px 20px 14px",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,background:C.bg,zIndex:50}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontSize:20,fontWeight:700,color:C.white}}>🏰 {data.settings?.allianceName||"Sunfire Command"}</div>
      ...
      {tab===0&&(
        <div style={{padding:"16px 20px 0"}}>
          ...
          {/* By Role view — previously the Teams tab */}
          {rosterView==="roles"&&(()=>{
      ...
      {/* ── Tab 1: Battle (placeholder — wire SvsPlanTab here) ── */}
      {tab===1&&(
        <div style={{padding:"40px 20px",textAlign:"center",color:C.muted}}>
          <div style={{fontSize:40,marginBottom:12}}>⚔️</div>
          <div style={{fontSize:16,fontWeight:700,color:C.white,marginBottom:8}}>Battle Plans</div>
          <div style={{fontSize:14,color:C.muted}}>Wire up SvsPlanTab here from the previous build.</div>
        </div>
      )}
      ...
      {/* ── Tab 4: Intel ── */}
      {tab===4&&<StatsTab players={players} events={events} onHeroClick={hero=>{setHeroFilter(hero);setHeroSheetOpen(true);}} onHeroOwnership={()=>setJoinerRegistryOpen(true)}/>}
      ...
      {joinerRegistryOpen&&(
        <div style={{position:"fixed",inset:0,zIndex:600,background:C.bg}}>
          <JoinerRegistry
            players={players}
            onUpdatePlayer={player=>{setData(prev=>({...prev,players:prev.players.map(p=>p.id===player.id?player:p),lastUpdated:new Date().toISOString()}));showToast("Saved ✓");}}
