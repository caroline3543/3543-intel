 ...
 *
 * 'xlsx' is imported statically, matching exportXlsx.js, rather than
 * via a dynamic import() — the library is already eagerly bundled
 * through the export path, so a dynamic import here bought no real
 * code-splitting benefit while adding a real failure mode ("Failed to
 * fetch dynamically imported module") when that separate chunk didn't
 * load.
 */
...
export async function importFromXlsxFile(file) {
  ...
  try {
    ...
    return { players, events, svsPlans, customRoles };
  } catch (err) {
    throw new Error('Could not read this spreadsheet — make sure it was exported from this app.');
  }
}
