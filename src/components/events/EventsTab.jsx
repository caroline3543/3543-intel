...
import { fmtDateShort, uid } from '../../utils/dates.js';
import { newEvent, newSnapshot } from '../../data/playerSchema.js';
import { Field, Inp, Sel, ToggleRow, SheetHandle } from '../common/Primitives.jsx';
...
function EventSheet({ event, open, onClose, onSave, players }) {
  ...
  const lastOpen = useState(open);
  if (open !== lastOpen[0]) { lastOpen[0] = open; if (open) setEv(event?{...event}:newEvent()); }
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:C.card, borderRadius:'20px 20px 0 0', width:'100%', maxHeight:'88vh', overflowY:'auto', padding:'16px 20px 80px' }}>
        ...
        <Field label="Participating Players" hint="Select who's in this event">
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {players.map(p=>{const sel=(ev.participantIds||[]).includes(p.id);return(
...
function SnapshotEditor({ snapshot, playerName, open, onClose, onSave }) {
  ...
  const lastOpen = useState(open);
  if (open !== lastOpen[0]) { lastOpen[0]=open; if(open&&snapshot) setS({...snapshot}); }
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:C.card, borderRadius:'20px 20px 0 0', width:'100%', maxHeight:'92vh', overflowY:'auto', padding:'16px 20px 100px' }}>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {PERF_TAGS.map(t=><button key={t.key} onClick={()=>setTag(t.key)} style={{ padding:'8px 14px', borderRadius:20, minHeight:36, border:`1px solid ${s.performanceTag===t.key?t.color:C.border}`, background:s.performanceTag===t.key?t.color+'18':C.section, color:s.performanceTag===t.key?t.color:C.muted, fontWeight:600, fontSize:13, cursor:'pointer' }}>{t.label}</button>)}
          </div>
        </div>
    ...
    <div style={{ padding:'16px 20px 0' }}>
      {activeEvent ? (
        ...
        <>
          ...
          {sorted.length===0?<div style={{ textAlign:'center', padding:'60px 20px' }}><div style={{ fontSize:40, marginBottom:12 }}>📋</div><div style={{ fontSize:16, fontWeight:700, color:C.white }}>No events yet</div></div>:sorted.map(ev=>{
              ...
              <div key={ev.id} onClick={()=>setActiveEventId(ev.id)} style={{ background:C.card, borderRadius:12, padding:'14px 16px', marginBottom:10, cursor:'pointer', border:`1px solid ${ev.status==='active'?C.green+'44':C.border+'44'}`, WebkitTapHighlightColor:'transparent' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4 }}>
                    ...
                    <button onClick={e=>{e.stopPropagation();if(window.confirm('Delete?'))onDeleteEvent(ev.id);}} style={{ fontSize:11, color:C.red+'88', background:'none', border:'none', cursor:'pointer' }}>Delete</button>
                  </div>
                </div>
